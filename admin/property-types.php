...
if(isset($_POST['title'])){
	...
	$query00 = mysqli_query($conn,"INSERT INTO `property_type`(`type`) VALUES ('$title')");

	$echo = "Added Successfully";
	
}
$query = mysqli_query($conn,"SELECT * FROM property_type ORDER BY tid DESC");
...
$i++;
	$fulldata .= "<tr class='todo-list-item'>
					...
					<td>".$i."</td>
					<td><input type='text' value='".$fetch_data['type']."' class='form-control feature-title".$fetch_data['tid']."' disabled p='".$fetch_data['tid']."'></td>
					<td><a href='#' class='f-edit' p='".$fetch_data['tid']."'>
									<em class='fa fa-edit'></em>
								</a></td>
					<td><a href='#' class='f-trash' p='".$fetch_data['tid']."'>
								...
								<div class="panel-body">
									<form action='' method='post'>	
						    			<div class='col-md-6'>
											...
											<input type='submit' class='btn btn-sm btn-info' value='Add Type'>
						        		</div>
						        	</form>	
								</div>
							</div><!-- /.panel-->
...
<script type="text/javascript">
	$(document).ready(function(){
		$(document).on('click','.f-edit',function(){
			...
			$(".feature-title"+tid).prop("disabled", false);
		})
		$(document).on('click','.f-update',function(){
			var tid = $(this).attr('p');
			$(this).removeClass('f-update').addClass('f-edit').html('<em class="fa fa-edit"></em>');		
			$(".feature-title"+tid).prop("disabled", true);
			var ttitle = $(".feature-title"+tid).val(); 
			$.ajax({
			      type: 'POST',
			      url: "edit-feature.php",
			      data: {Tid:tid,Ttitle:ttitle},
			      dataType: "text",
